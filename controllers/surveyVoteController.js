...
const table = "survey_vote"; //! Tablo Adı
...
exports.DataAddToken = async(req,res) => {
    ...
    try {
        ...
        //! Tanım
        const postAll = req.body; //! Gelen Veriler
        const token = req.body.voter_token; //! Token
        const decoded = jwt.verify(token, process.env.TokenSecret); //! Token Çözme
       
        //! Eklenecek Veriler
        delete postAll.voter_token; //! Sil
        postAll["voter_id"] = decoded?.id; //! Ekleme
        postAll["created_byId"] = decoded?.id; //! Ekleme

...
exports.DataAdd = async(req,res) => {
    ...
    try {
        ...
        */
        ...
        const dbFind = await dbModel.findByOther(table,info,req.query,search); //! Arama

        //! Veri Ekleme
        if(dbFind.result?.status == "success") { var result = { status :"error", rows: null, msg: "Email Kayıtlıdır", } }
        else {
            if(postAll?.password == postAll?.repassword) { delete postAll?.repassword; var result = await dbModel.createDB(table,postAll); } //! Model
...
exports.DataLogin = async(req,res) => {
    ...
    try {
        ...
        //! Tanım
        const postAll = req.body; //! Gelen Veriler
        const db = await dbModel.login(table,postAll); //! Model
        var token = null; //! Token

        if(db?.result?.status == "success" ) {

            //! Veriler
            const secret = process.env.TokenSecret;
            const data = {
                id: db?.result.rows[0].id,
                email: db?.result.rows[0].email,
            }
            var token = jwt.sign(data, secret); //! Token Oluşturma
        ...
        res.status(200).send({
            ...
            table: table,
            status: db?.result.status == "success" ? 1 : 0,
            msg: db.result?.status == "success" ? 'Veri Bulundu' : "Hata: "+db?.result.msg,
            token:token,
            DB:db
        });
